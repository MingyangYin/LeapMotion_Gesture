...
static const double DBL_MAX1 = std::numeric_limits<double>::max();
...
{
    tpl[STEP_DIAG] = (diag == NOT_VISITED) ? DBL_MAX1 : diag + diag_weight * local_cost;
    tpl[STEP_LEFT] = (left == NOT_VISITED) ? DBL_MAX1 : left + local_cost;
    tpl[STEP_UP] = (up == NOT_VISITED) ? DBL_MAX1 : up + local_cost;
                  ...
                  int window_size, int p, int diag_weight)
{
    ...
    std::vector<std::vector<double>> CM(2);
	CM[0].resize(ny + 1);
	CM[1].resize(ny + 1);
                    ...
                    std::vector<int>& idx, std::vector<int>& idy)
{
    ...
    std::vector<std::vector<double>> CM(nx+1);
	for (int i = 0; i <= nx;i++)
	{
		CM[i].resize(ny + 1);
	}
